...
public class ReflectionUtilsTest extends TestCase {
	...
	public void testAssertMethodArgs() throws Exception {
		...
		Method m = ReflectionUtilsTest.class.getDeclaredMethod("myMethod",
		...
		try {
			...
			assertTrue("Actual message: " + e.getMessage(), e.getMessage()
					.contains("expected 2 args, received 0"));
		}
		...
		try {
			...
			assertTrue(
					...
					e.getMessage()
							.contains(
									"arg 0 expected type class java.lang.String, got type class java.lang.Integer"));
		}
		...
		try {
			...
			assertTrue("Actual message: " + e.getMessage(), e.getMessage()
					.contains("arg 1 is a primitive, must not be null"));
		}
	...
	public void testAssignability() {
		...
		String[] methodNames = { "fByte", "fChar", "fShort", "fInt", "fLong",
				"fFloat", "fDouble" };
		Object[] values = { byteVal, charVal, shortVal, intVal, longVal,
				floatVal, doubleVal };
		...
		for (Object value : values) {
			...
			for (String methodName : methodNames) {
				...
				assertEquals(b, ReflectionUtils.isPromotableFrom(
	...
	public void testFieldAccess() {
		...
		ReflectionUtils.setFieldValue(flc,
				"secondLevelSimpleField.thirdLevelSimpleField", tlc);
		ReflectionUtils.setFieldValue(flc,
				"secondLevelSimpleField.thirdLevelSimpleField.value", value);
		assertSame(slc,
				ReflectionUtils.getFieldValue(flc, "secondLevelSimpleField"));
		assertSame(tlc, ReflectionUtils.getFieldValue(flc,
				"secondLevelSimpleField.thirdLevelSimpleField"));
		assertEquals(value, ReflectionUtils.getFieldValue(flc,
				"secondLevelSimpleField.thirdLevelSimpleField.value"));

		BigDecimal bd = new BigDecimal(value + 1);
		ReflectionUtils.setFieldValue(flc,
				"secondLevelSimpleField.thirdLevelSimpleField.value", bd);
		assertEquals(value + 1, ReflectionUtils.getFieldValue(flc,
				"secondLevelSimpleField.thirdLevelSimpleField.value"));
	}
	...
	public void testFieldAccessIllegalFieldNames() {
		...
		try {
			...
			assertTrue("Unexpected Exception",
					rex.getMessage().indexOf("Likely double \".\"") != -1
							&& rex.getMessage().indexOf("ending with") != -1);
		}
		...
		try {
			...
			assertTrue("Unexpected Exception",
					rex.getMessage().indexOf("Likely double \".\"") != -1
							&& rex.getMessage().indexOf("starts with") != -1);
		}
		...
		try {
			...
			assertTrue(
					...
					rex.getMessage().indexOf(
							"is the only part of the deepFieldName") != -1);
		}
		...
		try {
			...
			assertTrue("Unexpected Exception",
					rex.getMessage().indexOf("does not have field") != -1);
		}
		...
		try {
			ReflectionUtils.setFieldValue(new FirstLevelClass(),
					"secondLevelSimpleField.thirdLevelSimpleField",
					new ThirdLevelClass());
			assertFalse("Exception expected", true);
		} catch (RuntimeException rex) {
			assertTrue("Unexpected Exception",
					rex.getMessage().indexOf("is null") != -1);
		}
	...
	public void testGetDeclaredFieldInHierarchy() {
		...
		Field field = ReflectionUtils.getDeclaredFieldInHierarchy(Child.class,
				"field1");
	...
	public void testGetDeclaredFieldsInHierarchy() {
		...
		Field[] fields = ReflectionUtils
				.getDeclaredFieldsInHierarchy(Baz.class);
	...
	public void testInvokeByName() throws Exception {
		...
		// Happy path
		ReflectionUtils.invokeByName(this, "myMethod", "foo", 42);

		// Bad args
		try {
			ReflectionUtils.invokeByName(this, "myMethod", "foo");
			fail("Expected exception");
		} catch (Exception e) {
			assertTrue("Actual message: " + e.getMessage(), e.getMessage()
					.contains("expected 2 args, received 1"));
		}

		// Method not found
		try {
			ReflectionUtils.invokeByName(this, "myPoorBrain", "foo", 42);
			fail("Expected exception");
		} catch (Exception e) {
			assertTrue(
					"Actual message: " + e.getMessage(),
					e.getMessage().contains(
							"Method myPoorBrain not found in class "
									+ ReflectionUtilsTest.class.getName()));
		}

		// Private Method
		try {
			ReflectionUtils.invokeByName(this, "privateMethod", "foo", 42);
		} catch (Exception e) {
			assertTrue(
					"Actual message: " + e.getMessage(),
					e.getMessage().contains(
							"Method privateMethod not found in class "
									+ ReflectionUtilsTest.class.getName()));
		}

		// Exception thrown by method
		try {
			ReflectionUtils.invokeByName(this, "throwException");
			fail("Expected exception");
		} catch (Exception e) {
			assertTrue(e instanceof WrapperException);

			assertNotNull(e.getCause());
			assertTrue("Actual message: " + e.getCause().getMessage(), e
					.getCause().getMessage().equals("myexception"));
		}
	...
	public void testIsPromotableFrom() {
		...
		assertTrue(ReflectionUtils.isPromotableFrom(boolean.class,
				Boolean.class));
		assertTrue(ReflectionUtils.isPromotableFrom(Boolean.class,
				boolean.class));
		...
		assertTrue(ReflectionUtils
				.isPromotableFrom(char.class, Character.class));
		assertTrue(ReflectionUtils
				.isPromotableFrom(Character.class, char.class));
